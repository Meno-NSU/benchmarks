...
def judge_cases(
    settings: JudgeSettings, test_cases: TestCasesFileFull
) -> list[TestOut]:
    match settings:
    ...
    scorer = Rouge(metrics=("rouge-l",), stats="f")
    try:
        for case in tqdm.tqdm(test_cases):
            ...
            rouge_eval = scorer.get_scores(case["model_answer"], case["ground_truth"])

            results.append(
                TestOut(
                    result=TestMetricsResults(
                        **geval.eval(llm_test_case),
                        rouge_l_f1=rouge_eval.get("rouge-l", {"f": -1.0}).get(
                            "f", -1.0
                        ),
                    ),
                    case=case,
